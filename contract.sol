...
contract SSI {
    ...
    function approveClaimRequest(
        ...
        string[] memory signatures = new string[](approvals.length);

        for (uint i = 0; i < approvals.length; i++) {
            if (approvals[i].approved) {
                signatures[approvedCount] = approvals[i].approverDid;
                approvedCount++;
            }
        }

        if (approvedCount >= claim.numberOfApprovalsNeeded) {
            ...
            string[] memory finalSigs = new string[](approvedCount);

            for (uint i = 0; i < approvedCount; i++) {
                finalSigs[i] = signatures[i];
            }
